...
#define HEADER_SIGNATURE_B1     SUOTA_1_1_IMAGE_DA1469x_HEADER_SIGNATURE_B1
#define HEADER_SIGNATURE_B2     SUOTA_1_1_IMAGE_DA1469x_HEADER_SIGNATURE_B2
/* Get proper field from SUOTA header - DA1469x version */
#define FIELD(_da14680, _da1469x) _da1469x
...
__RETAINED static bool scan_any;
...
__STATIC_INLINE found_device_t *add_found_device(const bd_address_t *addr, size_t *index)
{
        found_device_t *dev;
...
static void initialize_nvms(void)
{
        ...
        if (header.FIELD(signature, image_identifier)[0] != HEADER_SIGNATURE_B1 ||
                        header.FIELD(signature, image_identifier)[1] != HEADER_SIGNATURE_B2) {
        ...
        image_size = header.FIELD(exec_location, pointer_to_ivt) + header.FIELD(code_size, size);
        ...
        if (timeinfo) {
                ...
                printf("\tTimestamp: INVALID TIMESTAMP\r\n");
        }
...
static void clicmd_connect_handler(int argc, const char *argv[], void *user_data)
{
        ...
        if (app_state != APP_STATE_IDLE) {
                printf("ERROR: connection failed, device needs to be in idle state "
                                                                                "to connect\r\n");
                return;
        }
...
static void clicmd_update_handler(int argc, const char *argv[], void *user_data)
{
        ...
        force_gatt = (argc > 1 && !strcmp("gatt", argv[1]));
